...
- uncomment the lines corresponding 
to a desired dataset (MNIST, NIST, FASHIONMNIST) and a seed in order to select a specific subset of the dataset.
- Set the number of independent tree formation processes and the range of the parameter _r_ at line 22 (or 23).
...
- To generate the tree structures for the NIST dataset uncomment the line 23 in `run_pipeline.sh`. The seed is irrelevant for the NIST-case.  

The script:
- `dataset_complexity_estimation.py` plots figure 2.
- `Portegys_plot_over_all_r.py` produces the same figure and analysis as `Portegys.py`, but addressing the full range of `r` for figure 5.a.
- `hierarchical_phase_transition.py` plots panels (d) and (e) in figure 5.
- `grid_of_spectral_entropy.sh` calls `grid_of_spectral_entropy.py`: takes in the path of a directory and plots the .jpg files contained in a tabular structure.
